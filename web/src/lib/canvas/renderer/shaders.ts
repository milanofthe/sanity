...
in vec4 aMeta;      // array layer, alpha, language family, recency
uniform mat3 uView;
out vec2 vUv;
flat out float vLayer;
flat out float vFade;
flat out vec3 vLang;
flat out float vHeat;
...
// How far the colour comes from the language rather than from the tokens, and
// how much a recent write brightens it. Both are zero until the zoom is far
// enough out that the tokens have stopped saying anything; see languageTint in
// lod.ts. No backticks in here: this GLSL is a template literal, so one ends
// the string and the error arrives as a parse failure in the TypeScript.
...
void main() {
  ...
  // And a file written a moment ago is brighter. Only out here: closer in, the
  // panel's border carries recency and doing both would be two answers to one
  // question.
  rgb = mix(rgb, vec3(1.0), vHeat * uLangTint * 0.45);
