...
for (const entry of REPOS) {
  ...
  // Their pages are rendered by the dump itself; see examples/dump.rs.
  const documents = scan.files.filter((f) => f.media && f.media.kind === 'document');
  ...
  console.log(
    `${entry.id}: ${scan.files.length} files, ${lines.toLocaleString('en-US')} lines, ` +
      `${pictures} pictures (${documents.length} documents), ` +
